...
class woxiaxiede_agg(transform):
	...
	def fit(self,data):
		...
		self.data_freq = pd.infer_freq(self.df.index)
	...
	def apply(self,time,weight = []):
		if weight:
			...
			self.weight = self.compute_weight(self.df.index,self.data_freq,time)
			# print(self.weight)
			# print(self.df[0:-1])
			self.avg_df = self.df[0:-1]/self.weight
			self.avg_df = self.append_index(self.avg_df)
			# print(self.avg_df)
			# self.avg_df[]
			...
			self.converted_df = model.predict('D')
	...
	def compute_weight(self,index,freq,final_freq):
		diff =[]
